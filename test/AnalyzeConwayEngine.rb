...
if __FILE__ == $0
  ...
  runs = 100
  bm_result = Benchmark.measure {  runs.times {freshConwayEngine.iterate()} }
  while true do
    freshConwayEngine.printCells(freshConwayEngine.cells)
    puts "Just did #{runs} ticks in #{bm_result.real} secs, we're running about #{runs/bm_result.real} ticks/sec" unless runs==0
